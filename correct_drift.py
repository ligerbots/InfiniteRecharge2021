...
class CorrectDrift:
    ...
    def __init__(self):
        ...
        self.original_points_nt.addListener(lambda entry, key, value, param: self.update_original_points(), NetworkTablesInstance.NotifyFlags.UPDATE)
        self.modified_points_nt.addListener(lambda entry, key, value, param: self.update_modified_points(), NetworkTablesInstance.NotifyFlags.UPDATE)
    ...
    def extract_pts(self,nt):
        arr=nt.getDoubleArray([])
        if(len(arr)<6):
            return np.zeros((0,2))
        else:
            arr_np=np.array(arr)
            ...
            all = np.concatenate((start,center,end))
            return(m_to_in(all.reshape((-1,2))))

    def extract_pts_and_rot(self,nt):
        arr=nt.getDoubleArray([])
        if(len(arr)<6):
            return []
        else:
    ...
    def combine_pts_and_rot(self,dat):
        res=[]
        for x in dat:
    ...
    def update_modified_points(self):
        self.points = self.extract_pts_and_rot(self.modified_points_nt)
        self.update_plot()
    ...
    def init_plot(self):
        ...
        self.btransformlater.on_clicked(self.transformlaterpressed)
    ...
    def update_plot(self):
        ...
        self.mod_arrow = self.ax.quiver(axs, ays, dxs, dys, zorder=3, angles='xy', scale_units='xy', scale=1, width=.007, color="green")
        self.figure.canvas.draw_idle()

    def transformlaterpressed(self, event):
                ...
                if self.drag_arrow:
                    if self.transformlater:
                        ...
                        for i in range(self.drag_pt,len(self.points)):
                            ...
                            this_ang+=delta
                            self.points[i]["position"]=np.array([this_d*math.cos(this_ang),this_d*math.sin(this_ang)])+pt
